...
Em que $a^\prime=\frac{1}{2}(a/c) $ e\textbf{ c }é o número de comparações duas a duas. O quantil $t_{(a,N-k)}$ é da distribuição t-Student com parâmetros N-k. Assim a margem de erro da equação anterior depende do número de comparações.
...
\[
\begin{cases}
 \text{$H_0$: As variáveis X e Y são independentes}\\
 \text{$H_1$: As variáveis X e Y não são independentes}
\end{cases}\\
\]
